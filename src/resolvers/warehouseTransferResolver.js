...
export const warehouseTransferResolver = {
  Query: {
    getWarehouseTransfersWithPagination: async (
      _,
      { page = 1, limit = 5, pagination = true, keyword = "", status, shopId },
      {user}
    ) => {
      requireAuth(user);
      ...
      try {
        const query = {
          ...
          ...(keyword
            ? {
                $or: [{ note: { $regex: keyword, $options: "i" } }],
              }
            : {}),
        };
          ...
          populate: [
            {
              ...
              populate: ["unitId", "parentProductId"],
            },
            "toShop",
            "requestedBy",
            "acceptedBy",
  ...
  Mutation: {
      ...
      try {
        ...
        if (!toShopIds || toShopIds.length === 0) {
          return errorResponse(
            "No destination shops selected",
            "មិនបានជ្រើសរើសហាងគោលដៅ",
          );
        }
        ...
        for (const it of items) {
          const totalQty =
            (totalBySub.get(it.subProductId) || 0) +
            it.quantity * toShopIds.length;
        ...
        for (const [subProductId, totalQty] of totalBySub.entries()) {
          ...
          if (!wh || wh.stock < totalQty) {
            return errorResponse(
              "Not enough stock in main warehouse for multi-shop transfer",
              "ស្តុកមិនគ្រប់គ្រាន់សម្រាប់ផ្ទេរទៅហាងជាច្រើន",
            );
          }
        }
    ...
    acceptWarehouseTransfer: async (_, { transferId, items }, { user }) => {
      try {
        requireAuth(user);

        const transfer = await WarehouseTransfer.findById(transferId);
        if (!transfer)
          return errorResponse("Transfer not found", "រកមិនឃើញការផ្ទេរ");
        if (!(transfer.status === "pending" || transfer.status === "partial_accepted")) {
          return errorResponse(
            "Transfer already processed",
            "ការផ្ទេរបានដំណើរការហើយ",
          );
        }

        const receiveMap = new Map();
        items.forEach((i) => {
          receiveMap.set(i.subProductId.toString(), i.receivedQty);
        });
        ...
        for (const item of transfer.items) {
          ...
          const receivedQty = receiveMap.get(sId) || 0;

          const currentReceived = item.receivedQty || 0;
          const remaining = item.quantity - currentReceived;

          if (receivedQty < 0 || receivedQty > remaining) {
            return errorResponse("Invalid quantity", "ចំនួនមិនត្រឹមត្រូវ");
          }

          if (receivedQty === 0) continue;

          let shopWh = await WarehouseInShop.findOne({
            shop: transfer.toShop,
            subProduct: sId,
          });

          if (!shopWh) {
            shopWh = await WarehouseInShop.create({
              shop: transfer.toShop,
              subProduct: sId,
              stock: 0,
              minStock: 0,
          ...
          shopWh.stock = previousStock + receivedQty;
          await shopWh.save();
          ...
          item.remainingQty = item.quantity - item.receivedQty;
          totalReceived += receivedQty;
        }

        if (totalReceived === 0) {
          return errorResponse("Nothing received", "មិនមានទំនិញទទួលចូល");
        }

        const hasRemaining = transfer.items.some((i) => i.remainingQty > 0);
        transfer.status = hasRemaining ? "partial_accepted" : "accepted";
        transfer.acceptedBy = user._id;
        transfer.acceptedAt = new Date();

        await transfer.save();

        return successResponse("Transfer accepted", "ទទួលការផ្ទេររួចរាល់");
      } catch (e) {
